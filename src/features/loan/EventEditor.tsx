...
export function readEventForm(values: FormData, scenario: Scenario, kind: LoanEvent['kind'], id: string, editing = false) {
  const event = { id, kind, month: Number(values.get('month')),
    ...(kind !== 'missed' ? { amount: parseSpanishNumber(String(values.get('amount') ?? '')) } : {}),
    ...(kind === 'recurring' ? { endMonth: Number(values.get('endMonth')), every: Number(values.get('every')) } : {}) };
  return scenarioSchema.safeParse({ ...scenario, events: editing
    ? scenario.events.map(item => item.id === id ? event : item) : [...scenario.events, event] });
}
...
export default function EventEditor({ scenario, month, onChange }: { scenario: Scenario; month: number; onChange: (value: Scenario) => void }) {
  ...
  function save(form: HTMLFormElement) {
    ...
    const next = readEventForm(values, scenario, kind, editing?.id ?? crypto.randomUUID(), Boolean(editing));
    if (!next.success) { setError('Revisa los meses (1 a 1.200) y el importe positivo, con coma decimal y máximo dos decimales. El mes final debe ser igual o posterior al inicial.'); return; }
  ...
  return <section aria-labelledby="events-title" className="event-editor">
    ...
    <form key={`${editing?.id ?? 'new'}-${month}`} aria-describedby={error ? 'event-error' : undefined} onSubmit={event => { event.preventDefault(); save(event.currentTarget); }}>
      ...
      <button className="button" type="submit">{editing ? 'Guardar evento' : 'Agregar evento'}</button>
      {editing && <button className="button ghost" type="button" onClick={() => { setEditing(null); setError(''); }}>Cancelar edición</button>}
    </form>
