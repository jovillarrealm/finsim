...
test('préstamo, edición de evento, exportación y persistencia mantienen resultados', async ({ page }) => {
  ...
  await expect(summary).toContainText('$ 507,51');
  await expect(summary).toContainText('$ 1.015,02');
  ...
  await expect(summary).toContainText('$ 1.014,02');
  ...
  await expect(summary).toContainText('$ 1.013,02');
...
test('móvil, teclado, conciliación y conversión funcionan con referencia independiente', async ({ page }) => {
  ...
  await expect(secondMonth).toContainText('$ 210,00');
  await expect(secondMonth).toContainText('$ 820,00');
  ...
  await expect(page.getByRole('table')).toContainText('$ 1,25');
